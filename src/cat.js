...
import pkg from 'encoding'; // Use default import for CommonJS modules
const { base16 } = pkg;
import { TextEncoder, TextDecoder } from 'util'; // Use native Node.js util for encoding/decoding
    ...
    static async validateEnc(value, catOptions, payload, request, decryptionKey) {
        ...
        if (decryptionKey) for (const [k, v] of value) {
            ...
            try {
                ...
                console.warn(`Failed to validate claims ${claimsString} from enc-label[${ClaimsLabelMap.enc}], reason=${error.message}`);
            }
        }
        return Promise.resolve({
            status: !0
